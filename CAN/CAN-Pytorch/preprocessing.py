...
def mask_test_edges(adj, testing=False):
    ...
    false_edges_dic = {}
    num_test = int(np.floor(len(edges) / 10.)) if not testing else 0
    num_val = int(np.floor(len(edges) / 20.)) if not testing else 0
...
def mask_test_feas(features, testing=False):
    ...
    false_feas_dic = {}
    num_test = int(np.floor(len(feas) / 10.)) if not testing else 0
    num_val = int(np.floor(len(feas) / 20.)) if not testing else 0
